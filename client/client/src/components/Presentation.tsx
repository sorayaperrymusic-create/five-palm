...
export default function Presentation() {
  ...
  return (
    <div className={`w-screen h-screen bg-black flex flex-col overflow-hidden ${
      ...
      {/* Slide Image Container - Responsive with Mobile Zoom */}
      <div className={`flex items-center justify-center bg-black overflow-hidden ${
        isMobilePortrait ? 'flex-1' : 'flex-1'
      }`}
      style={isMobilePortrait ? { zoom: '1.2' } : {}}>
        <img
          src={slide.backgroundImage}
          alt={`Slide ${currentSlide + 1}`}
          className={`${
            isMobilePortrait ? 'w-full h-full object-cover' : 'w-full h-full object-contain'
          }`}
          loading="lazy"
        />
      </div>
      ...
      <div className={`bg-black border-t border-cyan-400/20 ${
        ...
        <div className={`text-center text-cyan-300 ${
          isMobilePortrait ? 'text-xs leading-tight' : 'text-sm'
        }`}>
          <p>{isMobilePortrait ? '← → M' : '← → Space M'} • Slide {currentSlide + 1} / {slides.length}</p>
        </div>
      </div>
